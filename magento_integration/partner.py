...
class Partner(osv.Model):
    ...
    def find_or_create(self, cursor, user, customer_data, context):
        ...
        :return: Browse record of record created/found
        """
        if context.get('default_template_id',False):
            return super(Partner,self).find_or_create(cursor, user, customer_data, context)
        if not context['magento_website',False]:

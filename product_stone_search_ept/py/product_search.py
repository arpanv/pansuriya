...
###########################                        
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        def make_search(model,column,inner_type,model_type,no=1,range=False):
...
####                        
                ...
                while tmp != 0:
                    ...
                    for f in fields:
                        from_val = 0.0
                        if str(tmp) in f:
                            if model + '_from' in f and vals[f] :
                                search_domain.append(('&'))
                                search_domain.append((column, '>=', vals[f]))
                                from_val = vals[f]
                                break;
        ...
        info = [
                ...
                {'model':None,'_column_name':'product_price_caret','label':'PPC','no':1,'type':'float','name':'price_caret','product_search_type':'char','range':True},
                {'model':None,'_column_name':'product_discount','label':'Discount','no':1,'type':'float','name':'discount','product_search_type':'char','range':True},
        ...
        for model_info in info :
            make_search(model_info['_column_name'],model_info['name'],model_info['type'],model_info['product_search_type'], no=model_info.get('no') or 1, range=model_info.get('range') or False)
